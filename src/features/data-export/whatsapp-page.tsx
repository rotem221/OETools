...
import { MessageCircle, Download, Lock, Paperclip, ArrowLeft } from "lucide-react";
...
export function WhatsAppPage() {
  ...
  const { data: chats, isLoading } = useQuery({
    queryKey: ["wa-chats", source?.path],
    enabled: !!source && !source.encrypted,
    queryFn: async () => {
      const res = await api.listWaChats(source!.path);
  ...
  const { data: messages } = useQuery({
    queryKey: ["wa-chat", source?.path, active?.chat_id],
    enabled: !!source && !!active,
    queryFn: async () => {
      const res = await api.getWaChat(source!.path, active!.chat_id);
      if (!res.success || !res.data) throw new Error(res.error?.message);
      return res.data;
    },
  });

  const exportChat = async (format: "html" | "csv") => {
    if (!source || !active) return;
    const dest = await api.selectFolder();
    if (!dest.success || !dest.data) return;
    const res = await api.exportWaChat(source.path, active.chat_id, dest.data, format);
        ...
        <div className="grid gap-4 lg:grid-cols-[320px_1fr]">
          <Card>
            <CardContent className="p-2">
              {isLoading ? (
                ...
                <EmptyState className="border-0" icon={<MessageCircle className="h-6 w-6" />} title={t("dataExport.noWhatsapp")} />
              ) : (
                <ScrollArea className="h-[60vh]">
                  <div className="space-y-1 pr-2">
                    {chats.map((th) => (
                      ...
                      >
                        <p className="truncate text-sm font-medium">{th.display_name}</p>
          ...
          <Card>
            <CardContent className="flex h-[60vh] flex-col p-0">
              {!active ? (
                <EmptyState className="h-full border-0" icon={<MessageCircle className="h-6 w-6" />} title={t("messages.selectConversation")} />
              ) : (
                <>
                  <div className="flex items-center justify-between gap-2 border-b border-border p-3">
                    ...
                    <div className="flex items-center gap-2">
                      <Button variant="outline" size="sm" onClick={() => exportChat("html")}>
                        <Download className="h-4 w-4" /> HTML
                      </Button>
                      <Button variant="outline" size="sm" onClick={() => exportChat("csv")}>
                      ...
                      {(messages ?? []).map((m) => (
                        <div
                          ...
                          className={cn(
                            ...
                            m.from_me ? "ml-auto bg-primary text-primary-foreground" : "bg-muted",
                          )}
                        >
